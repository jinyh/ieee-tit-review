...
1. **移除人工评审时间参数**
   - 四阶段工作流程不再包含固定时间限制
   - 改为任务驱动的阶段式工作流，以完成标准判断进度
   - 参考 nsfc-i2-review 项目的设计方法

2. **新增评审模式选择**
   - 快速评审模式：25 项核心检查
   - 标准评审模式：465+ 项完整检查

3. **每个阶段添加明确的完成标准 (Completion Criteria)**
   - 阶段 1: 四项检查均完成，给出判断
   - 阶段 2: 所有检查清单项目均覆盖，每个定理/证明均检查
   - 阶段 3: 报告通过 quality-checklist.md 自检
   - 阶段 4: 每个 Major/Minor issue 逐项核实
